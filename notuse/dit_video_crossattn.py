...
class AdaLNMixin(BaseMixin):
    ...
    def layer_forward(
        ...
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp \
                = adaLN_modulation(kwargs['emb']).chunk(6, dim=1)
    ...
    def attention_fn(self, query_layer, key_layer, value_layer, attention_mask,
        ...
        if self.qk_ln:
            if "cross_attention" in kwargs.keys() and kwargs["cross_attention"] == True:
                query_layernorm = self.cross_query_layernorm_list[kwargs['layer_id']]
                key_layernorm = self.cross_key_layernorm_list[kwargs['layer_id']]
            else:
...
class DiffusionTransformer(BaseModel):
    def __init__(
        ...
        **kwargs
        ...
        if use_SwiGLU:
            self.add_mixin('swiglu', SwiGLUMixin(num_layers, hidden_size, self.inner_hidden_size, bias=False), reinit=True)
    ...
    def _build_modules(self, module_configs):
        ...
        if self.input_time == 'adaln':
            ...
            self.add_mixin('adaln_layer', instantiate_from_config(adaln_layer_config, hidden_size=self.hidden_size, num_layers=self.num_layers, 
