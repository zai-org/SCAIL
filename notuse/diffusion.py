...
class SATDiffusionEngine(nn.Module):
    def __init__(self, args, **kwargs):
        ...
        if isinstance(self.sampler, SeededNoiseEDMSampler):
            self.seeded_noise = SeededNoise(seeds=[seed+1234 for seed in self.sampler.seeded_noise.seeds], weights=self.sampler.seeded_noise.weights)

        self.use_lcm = model_config.get('use_lcm', False)
        if self.use_lcm:
            ...
            self.ema_model = copy.deepcopy(self.teacher_model)
            self.ema_model.eval()
    ...
    def forward(self, x, batch):
        if self.use_lcm:
            loss = self.loss_fn(self.model, self.denoiser, self.conditioner, x, batch, self.teacher_model, self.ema_model, self.sampler)
        else:
    ...
    def sample_lcm(
        ...
        **kwargs,
        ...
        denoiser = lambda input, sigma, c: self.denoiser(
            self.teacher_model, input, sigma, c, **kwargs
        )
        inter_samples = self.sampler(denoiser, randn, cond, uc=uc, distill=True)

        sigmas = self.sampler.discretization(
            50, device=self.device
        )
        uc = default(uc, cond)
        s_in = inter_samples.new_ones([inter_samples.shape[0]])
        lcm_denoiser = lambda input, sigma, c: self.denoiser(
            self.ema_model, input, sigma, c, **kwargs
        )

        lcm_steps = [20+i*(30//2) for i in range(2)]
        for step, step_next in zip(lcm_steps, lcm_steps[1:]+[-1]):
            inter_samples = self.sampler.sampler_step(s_in*sigmas[step], s_in*sigmas[-1], lcm_denoiser, inter_samples, cond, uc, 0.0)
            inter_samples += s_in * sigmas[step_next] * torch.randn_like(inter_samples)
    ...
    def log_images(
        ...
        **kwargs,
        ...
        if sample and isinstance(self.loss_fn.sigma_sampler, PartialDiscreteSampling):
            ...
            log["samples"] = samples
        elif sample and self.use_lcm:
            samples = self.sample_lcm(
