...
if __name__ == '__main__':
    ...
    for subdir_idx, subdir in tqdm(enumerate(sorted(os.listdir(evaluation_dir)))):
        ...
        out_path = os.path.join(evaluation_dir, subdir, 'smpl_hybrid_aligned_reshaped.mp4')
        out_ori_path = os.path.join(evaluation_dir, subdir, 'smpl_hybrid_aligned.mp4')
        ...
        if os.path.exists(poses_cache_path) and os.path.exists(det_cache_path) and os.path.exists(nlf_cache_path):
            ...
            # mpy.ImageSequenceClip(frames_ori_np, fps=16).write_videofile(out_ori_path)
        else:
