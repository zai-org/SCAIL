...
from ..util import (default, get_nested_attribute, get_obj_from_str,
                    ...
                    get_context_parallel_src_rank, is_context_parallel_initialized)
...
class VideoAutoencodingEngine(AutoencodingEngine):
    ...
    def get_input(self, batch: dict) -> torch.Tensor:
        if self.context_parallel_size > 1:
            ...
            src_rank = get_context_parallel_src_rank()
            torch.distributed.broadcast(batch, src=src_rank, group=get_context_parallel_group())
...
class VideoAutoencoderInferenceWrapper(VideoAutoencodingEngine):
    def __init__(
        ...
        assert self.cp_size == 1, "Only support cp_size=1"
    ...
    def encode(
        ...
        if self.cp_size > 0 and use_cp and not input_cp:
            ...
            src_rank = get_context_parallel_src_rank()
            torch.distributed.broadcast(x, src=src_rank, group=get_context_parallel_group())

            x = _conv_split(x, dim=2, kernel_size=1)

        frame = x.shape[2]
        temporal_compress_times = self.encoder.temporal_compress_times
        latent_time = (frame - 1) // temporal_compress_times + 1
        fake_cp_size = min(6, latent_time // 2)
        if fake_cp_size == 0:
            fake_cp_size = 1
        ...
        for i in range(fake_cp_size):
            end_frame = start_frame + (latent_time // fake_cp_size + (
                1 if i < latent_time % fake_cp_size else 0)) * temporal_compress_times
            if i == 0:
                end_frame = end_frame - temporal_compress_times + 1

            fake_cp_rank0 = True if i == 0 else False
            clear_fake_cp_cache = True if i == fake_cp_size - 1 else False
            with torch.no_grad():
                z = super().encode(x[:, :, start_frame:end_frame].contiguous(), return_reg_log,
                                    clear_fake_cp_cache=clear_fake_cp_cache, fake_cp_rank0=fake_cp_rank0)
                if return_reg_log:
                    z, reg_log = z
            latents.append(z)
    ...
    def decode(
        ...
        if self.cp_size > 0 and use_cp and not input_cp:
            ...
            src_rank = get_context_parallel_src_rank()
            torch.distributed.broadcast(z, src=src_rank, group=get_context_parallel_group())

            z = _conv_split(z, dim=2, kernel_size=1)

        latent_time = z.shape[2]
