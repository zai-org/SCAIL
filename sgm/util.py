...
def get_context_parallel_src_rank():
    return torch.distributed.get_process_group_ranks(get_context_parallel_group())[0]
...
def check_value_sync(x: torch.Tensor, name: str = ''):
    from sat import mpu
    db_rank = mpu.get_data_broadcast_rank()
    if db_rank == 0:
        gather_list = [torch.zeros_like(x) for i in range(mpu.get_data_broadcast_world_size())]
    else:
        gather_list = None

    torch.distributed.gather(x, gather_list, dst=mpu.get_data_broadcast_src_rank(), group=mpu.get_data_broadcast_group())
    diff_list = []
    if db_rank == 0:
        for i, t in enumerate(gather_list):
            remote_rank = torch.distributed.get_rank() + i
            diff = (t - x).abs().sum()
            if diff > 0.:
                diff_list.append((torch.distributed.get_rank(), remote_rank))

    if db_rank == 0:
        if diff_list:
            print('Rank %d: Value %s diff detected between rank pairs:' % (torch.distributed.get_rank(), name), diff_list)
        else:
            print('Rank %d: Value %s sync check passed:' % (torch.distributed.get_rank(), name))
...
def append_one(x):
    return torch.cat([x, x.new_ones([1])])
