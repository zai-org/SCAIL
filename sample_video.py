        ...
        while True:
            ...
            if args.image2video:    # i2v 输入是图片+prompt
                ...
                if sampling_num_frames > 81:
                    raise ValueError("Too many frames in pose video, please provide <= 81 frames.")
                        ...
                        if mpu.get_model_parallel_rank() == 0:
                            if GT is not None:
                                if "smpl_downsample" in args.representation:
                                    ...
                                    save_list = [torch.clamp((smpl_render_video + 1.0) / 2.0, min=0.0, max=1.0).cpu(), torch.clamp((image_to_save + 1.0) / 2.0, min=0.0, max=1.0).cpu(), torch.clamp((GT.unsqueeze(0) + 1.0) / 2.0, min=0.0, max=1.0).cpu()]
                                save_multi_video_grid_and_mp4(save_list + [samples], save_dir, fps=args.sampling_fps, key=f"{test_case_idx}_concat") # 都要求是B T C H W
                            save_multi_video_grid_and_mp4([samples], save_dir, fps=args.sampling_fps, key=f"{test_case_idx}_output")
