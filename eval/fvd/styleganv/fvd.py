...
def get_feats(videos, detector, device, bs=10):
    ...
    with torch.no_grad():
        for i in range((len(videos)-1)//bs + 1):
            processed_videos = [preprocess_single(video) for video in videos[i*bs:(i+1)*bs]]
            output = detector(torch.stack(processed_videos).to(device), **detector_kwargs)
            feats = np.vstack([feats, output.detach().cpu().numpy()])
    return feats
