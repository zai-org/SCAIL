...
class ChatGLM3UlyssesAttnMixin(BaseMixin):
    def __init__(self, hidden_size, num_heads, max_seq_len, base_scale):
        super().__init__()
        rotary_dim = hidden_size // num_heads
        self.rotary_pos_emb = FastRotaryEmbedding(rotary_dim // 2, interleaved=True, base=10000*base_scale)
    ...
    def attention_forward(self, hidden_states, mask, **kw_args):
        ...
        if type(attention_fn) == partial:
            fn = partial(attention_fn.func, None, None, None, mask, dropout_fn, **attention_fn.keywords, **kw_args)
        else:
            fn = partial(attention_fn, None, None, None, mask, dropout_fn, **kw_args)

        context_layer = origin.dist_attn(
...
class ChatGLM3Model(BaseModel):
    def __init__(self, args, transformer=None, **kwargs):
        ...
        if mpu.get_sequence_parallel_world_size() > 1:
            self.add_mixin("attn", ChatGLM3UlyssesAttnMixin(args.hidden_size, args.num_attention_heads, args.max_sequence_length, args.base_scale))
        else:
    ...
    def get_position_ids(self, input_ids):
        ...
        if mpu.sequence_parallel_is_initialized():
            seq_length = seq_length * mpu.get_sequence_parallel_world_size()
    ...
    def forward(self, input_ids, position_ids=None, attention_mask=None, past_key_values=None, **kwargs):
        ...
        if mpu.sequence_parallel_is_initialized():
            position_ids = torch.chunk(position_ids, mpu.get_sequence_parallel_world_size(), dim=1)[mpu.get_sequence_parallel_rank()]
